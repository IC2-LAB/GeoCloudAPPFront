...
function SM4() {
     ...
     this.sm4_crypt_ecb=function(ctx,input){
        ...
        if ((ctx.isPadding) && (ctx.mode == SM4_ENCRYPT)){
            input = this.padding(input, SM4_ENCRYPT);
        }
        ...
        {
            output = this.padding(output, SM4_DECRYPT);
        }
...
const secretKey="11HDESaAhiHHugDz"
const iv= ''
let hexString = false
export const SM4Util = {
  secretKey: "11HDESaAhiHHugDz",
  iv: '',
  hexString: false,
    encryptData_ECB: function(plainText){
        try{
            ...
            ctx.isPadding = true
            ctx.mode = SM4_ENCRYPT
            var keyBytes= SM4Util.stringToByte(SM4Util.secretKey)
            sm4.sm4_setkey_enc(ctx, keyBytes)
            var encrypted = sm4.sm4_crypt_ecb(ctx, SM4Util.stringToByte(plainText))
    ...
    decryptData_ECB: function(cipherText){
      ...
      var ctx = new SM4_Context()
			...
			return SM4Util.byteToString(decrypted)
		} catch (e) {
			// e.printStackTrace();
			return null;
		}
    ...
    stringToByte: function(str) {
        ...
        for (var i = 0; i < str.length; ) {
            ...
            if (codePoint > 0x10FFFF) {
                throw new Error("Code point out of valid UTF-8 range");
            }
            if (codePoint <= 0x7F) {
                // 1-byte sequence
            ...
            i += codePoint > 0xFFFF ? 2 : 1; // Increment by the number of code units in the character
        }
        return bytes;
    },
    ...
    byteToString: function (arr) {
        ...
        for (var i = 0; i < arr.length; ) {
            ...
            if ((byte >> 7) == 0) { // 1-byte sequence
                codePoint = byte;
                bytesLength = 1;
            } else if ((byte >> 5) == 0b110) { // 2-byte sequence
                codePoint = (byte & 0x1F) << 6;
                bytesLength = 2;
            } else if ((byte >> 4) == 0b1110) { // 3-byte sequence
                codePoint = (byte & 0x0F) << 12;
                bytesLength = 3;
            } else if ((byte >> 3) == 0b11110) { // 4-byte sequence
            ...
            // Read the continuation bytes and assemble the code point
            for (var j = 1; j < bytesLength; j++) {
                var nextByte = arr[i + j];
                if ((nextByte >> 6) != 0b10) {
                    throw new Error("Invalid continuation byte in UTF-8 sequence");
                }
                codePoint |= (nextByte & 0x3F) << (6 * (bytesLength - j - 1));
            }
    
            // Convert the code point to a string and append it
