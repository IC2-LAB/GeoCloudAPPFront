  ...
  <div>
    <van-steps direction="vertical" :active="1">
      <van-step v-for="(item, index) in imgArr" :key="index">
        <p>{{item.time}}</p>
        <video width="100%" :src="item.url" alt="" controls="controls" />
        <p>{{item.desc}}</p>
      </van-step>
    </van-steps>
    ...
    >
      ...
      />
      <van-divider />
      <van-uploader v-model="fileList" multiple :max-count="1" accept="video/*" />
      <van-divider />
...
export default {
  ...
  computed: {
    imgArr () {
      let {videonode, projectId} = this.dataAll
      let imgArr = []
      if (videonode) {
        let arrO = videonode.split('##***##')
        arrO.map(item => {
          let arrT = item.split('##%%%##')
          let obj = {}
          obj.time = arrT[0]
          obj.url = `/upload/${projectId}/video/${arrT[1]}`
          obj.desc = arrT[2]
          imgArr.push(obj)
        })
      }
  ...
  methods: {
    ...
    commitAdd () {
      ...
      //   forbidClick: true
      // })
      // http.uploadVideo(param).then(res => {
      //   // console.log(res)
