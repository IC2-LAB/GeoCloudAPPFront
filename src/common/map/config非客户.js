...
// 内网
let qzmap = process.env.NODE_ENV === 'production' ? ipUrlConfig().map : '/qzszlh/map'
let qzgeoserver = process.env.NODE_ENV === 'production' ? ipUrlConfig().geoserver : '/qzszlh/geoserver'
let qzlocalImageserver = process.env.NODE_ENV === 'production' ? ipUrlConfig().localImageserver : '/qzszlh/localImageserver'
let bendi = process.env.NODE_ENV === 'production' ? ipUrlConfig().bendi : '/qzszlh/bendi'
export default {
  'type': '2d',
  'engine': 'openlayers',
  'view': {
    center: [118.6869, 28.8845],
  ...
  'layers': [
    ...
    // }
    {
      "name": "影像",
      "layerName": "yx",
      "layerUrl": "tianditu.gov.cn/DataServer?T=img_c&x={x}&y={y}&l={z}&tk=6557db1c1a37d8ef0762d6efd494ab77",
      "layerType": "XYZOnlinetdt4326",
      "layerIcon":"",
      "visible": true
    },
    {
